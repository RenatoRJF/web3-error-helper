...
import { TranslateErrorOptions } from '../types';
...
describe('translateError', () => {
  ...
  describe('ERC20 errors', () => {
    ...
    it('should translate ERC20 transfer amount exceeds allowance', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Transfer amount exceeds your approved allowance. Please increase your token allowance first.'
      );
    });
    ...
    it('should translate ERC20 insufficient allowance', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Insufficient token allowance. Please approve more tokens before attempting this transaction.'
      );
    });
  });
  ...
  describe('Gas errors', () => {
    it('should translate gas required exceeds allowance', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        "Transaction requires more gas than you've allocated. Please increase your gas limit."
      );
    });
    ...
    it('should translate out of gas error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Transaction ran out of gas. Please increase your gas limit and try again.'
      );
    });
    ...
    it('should translate nonce too low error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Transaction nonce is too low. Please wait for previous transactions to be processed or reset your nonce.'
      );
    });
  });
  ...
  describe('Wallet errors', () => {
    it('should translate user rejected request', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Transaction was rejected by the user. Please try again and confirm the transaction in your wallet.'
      );
    });
    ...
    it('should translate wrong network error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        "You're connected to the wrong network. Please switch to the correct network in your wallet."
      );
    });
  });
  ...
  describe('Network errors', () => {
    ...
    it('should translate timeout error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Request timed out. The network may be congested. Please try again in a few moments.'
      );
    });
  });
  ...
  describe('Transaction errors', () => {
    ...
    it('should translate invalid address error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Provided Ethereum address is invalid. Please check the address format and try again.'
      );
    });
    ...
    it('should translate execution reverted error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Transaction execution was reverted. Please check the transaction details and try again.'
      );
    });
  });
  ...
  describe('EVM errors', () => {
    it('should translate invalid opcode error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'An invalid operation was executed. This may indicate a contract bug or compatibility issue.'
      );
    });
    ...
    it('should translate stack overflow error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Too many items on the EVM stack. This may indicate a contract execution issue.'
      );
    });
    ...
    it('should translate panic code 0x01', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Assertion failed. A condition that should never be false was violated.'
      );
    });
    ...
    it('should translate panic code 0x11', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Arithmetic overflow or underflow occurred. Please check the calculation values.'
      );
    });
  });
  ...
  describe('Contract errors', () => {
    it('should translate Ownable error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Function can only be called by the contract owner. Please contact the contract owner to perform this action.'
      );
    });
    ...
    it('should translate Pausable error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Contract is paused. This action cannot be executed at this time. Please try again later.'
      );
    });
    ...
    it('should translate SafeMath overflow error', () => {
      ...
      expect(result.translated).toBe(true);
      expect(result.message).toBe(
        'Arithmetic addition overflow. The result is too large to be stored.'
      );
    });
  });
